...
import { saveTrialSignup } from '../../../utils/api'
...
// Static icon map by priority
const PRIORITY_META = {
  1: { icon: '🎯', label: 'Priority 1 — start here' },
  2: { icon: '🌐', label: 'Priority 2 — add next' },
  3: { icon: '🔍', label: 'Priority 3 — future add' },
}

// Static system prompt template (same for all agents, agent_name injected)
const buildSystemPrompt = (agentName) => `You are ${agentName}, an AI assistant for this business.
...
FALLBACK:
"I don't have that specific information. Let me connect you with our team."`;
...
export default function RecommendationPage() {
  ...
  // Get static recommendation for this survey
  // const rec = survey_id ? recommendations[String(survey_id)] : null

  useEffect(() => {
    if (!session_id_param) return
  ...
  option_id: option_id
        ...
        if (data.code === 200 && !data.recommendation.error) {
          const agents = data.recommendation.recommended_agents
          ...
          setRec({
            ...
            agents: agents.map((agent) => ({
              ...
              label:        PRIORITY_META[agent.priority]?.label ?? `Priority ${agent.priority}`,
              icon:         PRIORITY_META[agent.priority]?.icon ?? '🤖',
              title:        agent.agent_name,        // ← API
  ...
  // Show loading state while router hydrates
  // if (!survey_id) {
  //   return (
  ...
  if (!survey_id || loading) {
    return (
      <Layout currentStep={4}>
        <div className="min-h-[calc(100vh-53px)] p-6 sm:p-10" style={{ background: '#1a1a1a' }}>
          <div className="animate-pulse space-y-4 max-w-7xl">
            <div className="h-4 bg-gray-700 rounded w-1/3" />
            <div className="h-10 bg-gray-700 rounded w-2/3" />
            <div className="h-72 bg-gray-800 rounded-2xl mt-6" />
          </div>
    ...
    return (
      <Layout currentStep={4}>
        <div className="p-10 text-gray-400">
          Unable to load your recommendation. Please try again.
        </div>
      </Layout>
    )
  }
  ...
  //   return (
  //     <Layout currentStep={4}>
  //       <div className="p-10 text-gray-400">No recommendation found for this survey.</div>
  //     </Layout>
  //   )
  // }

  // const agentCount = rec.agents.length
                ...
                {agent?.conversations?.length > 0 && (
                  <div className="bg-gray-50 rounded-xl p-4">
                    ...
                    <div className="space-y-3">
                      {agent?.conversations?.map((conv, i) => (
                        <div key={i} className="space-y-1.5">
                          <div className="flex items-start gap-2">
                            <span className="text-xs font-semibold text-gray-400 bg-gray-200 px-1.5 py-0.5 rounded flex-shrink-0">
                              User
                            </span>
                            <p className="text-sm text-gray-700">{conv.user}</p>
                          </div>
                          <div className="flex items-start gap-2">
                            <span className="text-xs font-semibold text-blue-600 bg-blue-100 px-1.5 py-0.5 rounded flex-shrink-0">
                              AI
                            </span>
                            <p className="text-sm text-gray-600">{conv.ai}</p>
                          </div>
                        </div>
                      ))}
                    </div>
